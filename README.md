# Privacidade
## Navegador
- Firefox + [arkenfox/user.js](https://github.com/arkenfox/user.js/)
- Mullvad Browser
- Tor Browser
## VPN
- [IVPN](https://www.ivpn.net)
- [Mullvad](https://mullvad.net/)
## Email 
- [Mailbox.org](https://mailbox.org)
- [Tuta](https://tuta.com/)
## Cliente de Email
- Thunderbird
## Criptografia
- OpenPGP
- GnuPG + Kleopatra
- OpenKeychain no Android
- VeraCrypt (Windows/Linux), criptografia de disco e de pastas/arquivos.
- Cryptomator (Windows/Linux/Android), criptografia de arquivos, os arquivos são criptografados separadamente e não somente em um bloco.
- RClone com qualquer serviço de armazenamento em nuvem.
## Nuvem 
- Nextcloud (Autohospedado)
- Tresorit
- Peergos
## Gerenciador de Senha
- [KeePassXC ](https://keepassxc.org/)+ [Nextcloud](https://nextcloud.com/)
- [Bitwarden](https://bitwarden.com/)
- [dani-garcia/vaultwarden](https://github.com/dani-garcia/vaultwarden) (Bitwarden, porém autohospedado)
## Rede social
- Lemmy, Session, Mastodon, XMPP e Matrix ([Element](https://element.io))
## XMPP
- Servidores que suportam OMEMO/OpenPGP
- [Gajim](https://gajim.org/), robusto cliente XMPP
- [Conversations](https://f-droid.org/en/packages/eu.siacs.conversations/), robusto cliente XMPP para Android
## Android 
- [GrapheneOS](https://grapheneos.org/)
- [LineageOS](https://lineageos.org/)
## Desktop
- Linux (distros como [Arch](https://archlinux.org/), [Qubes OS](https://www.qubes-os.org/), [Fedora](https://fedoraproject.org/))
- Live Boot em Flash Drive com [Whonix](https://www.whonix.org/)/[Tails OS](https://tails.net/)
## Provedor de dominio 
- [Sarek Oy](https://sarek.fi/)
- [Njal.la](https://njal.la/)
## Meet 
- [Jit.si](https://meet.jit.si/)
## Criptomoeda 
- [Monero (XMR)](https://www.getmonero.org/)
- [ZCash (ZEC)](https://z.cash/)
## Hospedagem
- [Servers.guru](https://servers.guru)
- [Njal.la](https://njal.la/)
## Motor de Pesquisa
- DuckDuckGo
- StartPage
## Roteador 
- Qualquer um que suporte [OpenWRT](https://openwrt.org/)
- [Pi-hole](https://pi-hole.net/) com lista do [oisd.nl](https://oisd.nl/)
- [pfSense](https://www.pfsense.org/)
## Depix
- [SpikeToSpike](https://spiketospike.com/)
- [AlfredP2P.io](https://www.alfredp2p.io)
